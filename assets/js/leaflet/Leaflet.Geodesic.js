...
L.Geodesic = L.Polyline.extend({
  ...
  initialize: function (latlngs, options) {
    ...
    this.options.dash = Math.max(
      1e-3,
      Math.min(1, parseFloat(this.options.dash) || 1),
    );
    this.datum = {};
  ...
  getStats: function () {
    ...
    for (poly = 0; poly < this._latlngs.length; poly++) {
      ...
      for (points = 0; points < this._latlngs[poly].length - 1; points++) {
        obj.distance += this._vincenty_inverse(
          this._latlngs[poly][points],
          this._latlngs[poly][points + 1],
        ).distance;
      }
    }
    return obj;
  ...
  geoJson: function (geojson) {
    ...
    for (let feature of features) {
      let geometry = feature.type === "Feature" ? feature.geometry : feature,
        coords = geometry.coordinates;

      switch (geometry.type) {
        case "LineString":
          this._latlngs.push(
            this._generate_Geodesic([L.GeoJSON.coordsToLatLngs(coords, 0)]),
          );
          break;
        case "MultiLineString":
        case "Polygon":
          this._latlngs.push(
            this._generate_Geodesic(L.GeoJSON.coordsToLatLngs(coords, 1)),
          );
          break;
        case "Point":
        case "MultiPoint":
  ...
  createCircle: function (center, radius) {
    ...
    let direct = this._vincenty_direct(
      L.latLng(center),
      0,
      radius,
      this.options.wrap,
    ...
    for (step = 1; step <= this.options.steps; ) {
      direct = this._vincenty_direct(
        L.latLng(center),
        (360 / this.options.steps) * step,
        radius,
        this.options.wrap,
      );
        ...
        for (let s = 1; s <= this.options.steps; ) {
          ...
          if (Math.abs(gp.lng - prev.lng) > 180) {
            ...
            if (this.options.dash < 1) {
              ...
              let direct_full = this._vincenty_direct(
                pointA,
                inverse.initialBearing,
                distance * s,
                this.options.wrap,
